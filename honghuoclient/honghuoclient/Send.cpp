...
void CSend::OnSize(UINT nType, int cx, int cy)
{
	...
	if( NULL != GetSafeHwnd() ) {
		...
		if ( NULL != pst ) {
			...
			pst->SetWindowPos( NULL ,136 , 56 , 333, rect.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_STATIC_XM ) ;
		if ( NULL != pst ) {
		CRect rect ;
		pst->GetClientRect( rect ) ;
		pst->SetWindowPos( NULL ,  136 + 333 + 80, 55 , rect.Width(), rect.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_EDIT_AIM_ADDRESS ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,136 , 100 , 333, 25  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_BUTTON_SELECT ) ;
		if ( NULL != pst ) {
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL ,136 + 333 + 10 , 96 , rect.Width(), rect.Height(),SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_EDIT_Lebel ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,136 , 144 , 333, 25  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_EDIT_MONEY ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,136 , 188 , 200, 25  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_COMBO_UNIT ) ;
		if ( NULL != pst ) {
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL ,336 + 10 , 188 , rect.Width(), rect.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pst = GetDlgItem( IDC_BUTTON_SEND ) ;
		if ( NULL != pst ) {
			pst->SetWindowPos( NULL ,136 + 10 , 230 , 81, 31  ,SWP_SHOWWINDOW ) ; 
		}
	}
}
...
BOOL CSend::Create(CWnd* pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID)
{
	...
	if ( bRes ){
		...
		m_strTx1.SetTextColor(RGB(255, 0, 0));
		...
		((CComboBox*)GetDlgItem(IDC_COMBO_UNIT))->AddString(_T("hhc"));
...
void CSend::OnBnClickedButtonSend()
{
	...
	CString strDisplay;
	strDisplay.Format(_T("%s%.4lfhhc%s%s"),_T("转账"), dSendMoney, _T("至") ,strMaddress);
