...
BEGIN_MESSAGE_MAP(ChuobiApp, CWinApp)
...
ChuobiApp::ChuobiApp()
{
	...
	m_strServerCfgFileName = "honghuo.conf";  //服务器配置文件名
...
BOOL ChuobiApp::RunOnlyOneApp()
{
	HANDLE hMutex = CreateMutex(NULL, FALSE, _T("HongHuoClient"));
	// 检查错误代码
...
BOOL ChuobiApp::CreateMaintainThrd() 
{
	hMtThrd		= NULL ;
	nMtThrdID	= 0  ;
...
UINT __stdcall ChuobiApp::MtProc(LPVOID pParam)
{
	MSG	msg , msgcpy ;
	PeekMessage(&msg, NULL, WM_USER, WM_USER, PM_NOREMOVE) ;

	if(!SetEvent( ((ChuobiApp*)pParam)->hMtStartEvent ) ) {
		return 1;
	}
	...
	{	
		...
		if ( MsgReturn )
		{
			...
			case MSG_USER_MOBILE_STATEUS:
				{
                    ::PostThreadMessage( ((ChuobiApp*)pParam)->GetMtHthrdId() , MSG_USER_MOBILE_TIP , msgcpy.wParam , msgcpy.lParam ) ;
				}
				break;
			case MSG_USER_QUITTHREAD:
				{
					std::vector< sThrd >::iterator it ;
					ChuobiApp* pApp = ((ChuobiApp*)pParam) ;
			...
			((ChuobiApp*)pParam)->DispatchMsg( ((ChuobiApp*)pParam)->GetMtHthrdId() , msgcpy.message , msgcpy.wParam , msgcpy.lParam ) ;
		}
	}
}
void ChuobiApp::RegThrdToMtThrd( HANDLE hThread , unsigned int hThrdId )
{
...
void ChuobiApp::UnRegThrdToMtThrd( HANDLE hThread , unsigned int hThrdId ) 
{
	if( NULL == hThrdId || NULL == hThread ) {
		return ;
	}
...
void ChuobiApp::SubscribeMsg( DWORD nThreadId , HWND hWnd, UINT msg , void * relMsgMem )
{
	if( NULL == hWnd ) {
		return ;
	}
...
void ChuobiApp::UnSubscribeMsg( DWORD nThreadId , HWND hWnd, UINT msg )
{
	if( NULL == hWnd ) {
		return ;
	}
...
void ChuobiApp::DispatchMsg( unsigned int threadID , UINT msg , WPARAM wParam , LPARAM lParam ) 
{
	BOOL flag = FALSE ;
	std::vector<MSG_NOTIFY>::iterator it ;
...
void ChuobiApp::GetMoFilename( string & path , string & filename ) 
{
...
void ChuobiApp::StartSeverProcess(const string& strdir)
{
	CloseProcess(_T("honghuo.exe"));
...
//	CString str;
//	str.Format(_T("honghuo.exe -datadir=%s -ui=1") ,strdir ) ;  //str = "honghuo.exe -datadir=D:\code\dacrs_ui_new\bin -ui=1"
	string startBat = strprintf("honghuo.exe -datadir=%s -ui=1", strdir);
	if(m_bReIndexServer)
...
void ChuobiApp::CloseProcess(const string& exename)
{
...
void ChuobiApp::OnCheckSeverfile()
{
	string strSeverPath = strprintf("%s\\honghuo.exe", m_strInsPath);
	...
	if (PathIsDirectory(strSeverPath.c_str()))
	{
		...
		{
			UiFun::MessageBoxEx(_T("honghuo 文件不存在,请重新启动钱包\r\n") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(1);
		}
	}

	strSeverPath = strprintf("%s\\temp\\honghuo.exe",m_strInsPath);
	...
	//if( (_access( strSeverPath.c_str(), 0 )) == -1 )
	//{
	//	UiFun::MessageBoxEx(_T("honghuo 文件不存在,请重新下载\r\n") , _T("Error") ,MFB_OK|MFB_ERROR );
	//	exit(1);
	//}
}

//校验路径合法性
void ChuobiApp::CheckPathValid(const string& strDir)
{
	BOOL bExist = FALSE;
...
void ChuobiApp::StartWorkThread() 
{
...
UINT __stdcall ChuobiApp::ReceiveMessageFromServer(LPVOID pParam)
{
	bool bReConnect(false);
	int nMaxbufferLen(10*1024-1);
	ChuobiApp * pHonghuoCoinApp  = (ChuobiApp*)pParam ;
	if ( NULL != pHonghuoCoinApp ) {
		while (true)
		{
			Sleep(100);
			...
			if(bReConnect) {
				...
				CSynchronousSocket te;
				SOCKET nSocket = te.OnblockConnnect((char*)pHonghuoCoinApp->m_severIp.c_str(),atoi(pHonghuoCoinApp->m_uiPort.c_str()));
				if( INVALID_SOCKET != nSocket) {
					pHonghuoCoinApp->m_uiConnSocket = nSocket;
					bReConnect = false;
				...
				if ( INVALID_SOCKET != pHonghuoCoinApp->m_uiConnSocket ) {
					char  cRecvbuffer[10*1024];
					int nRecLen = recv( pHonghuoCoinApp->m_uiConnSocket , cRecvbuffer , nMaxbufferLen , 0);
					...
					if ( nRecLen > 0 ) {
						...
						TRACE("\n");*/
						if(!pHonghuoCoinApp->m_noUiMsgBuffer.AddBytesToBuffer(cRecvbuffer, nRecLen)) {
							if (INVALID_SOCKET != pHonghuoCoinApp->m_uiConnSocket)
							{
								closesocket(pHonghuoCoinApp->m_uiConnSocket);
								pHonghuoCoinApp->m_uiConnSocket = INVALID_SOCKET;
							}
							pHonghuoCoinApp->m_noUiMsgBuffer.ClearBuffer();
							bReConnect = true;
						}
					}
					else if(nRecLen == 0) 
...
bool ProcessMsgJson(Json::Value &msgValue, ChuobiApp* pApp) 
{
...
UINT __stdcall ChuobiApp::PraseJsonToMessage(LPVOID pParam)
{
	ChuobiApp * pHonghuoCoinApp  = (ChuobiApp*)pParam;
	if ( NULL != pHonghuoCoinApp ) {
		while (true)
		{
		    Sleep(100); 
			if (theApp.m_bIsCloseAppFlag && !pHonghuoCoinApp->m_noUiMsgBuffer.HaveNoUiMsg())
			{
				return 1;
			}
			while(pHonghuoCoinApp->m_noUiMsgBuffer.HaveNoUiMsg()) {
				string strMsg;
				pHonghuoCoinApp->m_noUiMsgBuffer.GetNoUiMsg(strMsg);
				Json::Reader reader;  
				...
				if (!reader.parse(strMsg, jsonValue)) 
					continue;
				ProcessMsgJson(jsonValue, pHonghuoCoinApp);
			}
		}
	}
	return 1;
...
bool ChuobiApp::Update()
{
		...
		/*	if ( IDYES != UiFun::MessageBoxEx(sMsg, UiFun::UI_LoadString("COMM_MODULE" , "COMM_TIP" ,theApp.gsLanguage) , MFB_YESNO|MFB_TIP ) ){
			...
			if(m_pMainWnd != NULL)
				SendMessage(((ChuobiDlg*)(m_pMainWnd))->m_hWnd,MSG_USER_CLOSEPROCESS,NULL,NULL);
			CloseProcess("honghuo.exe");
			DWORD exc;
			...
			ShRun.nShow = SW_SHOW;
			ShellExecuteEx(&ShRun);
			CloseProcess("honghuoclient.exe");
			return true;
}


UINT __stdcall ChuobiApp::CheckUpdate(LPVOID lpParam){

	ChuobiApp * pApp  = (ChuobiApp*)lpParam; 
...
UINT __stdcall ChuobiApp::ProcessMessage(LPVOID pParam) {
	ChuobiApp * pApp  = (ChuobiApp*)pParam ;
	...
	while ( true)
	{
		...
		ChuobiDlg *pAppDlg = (ChuobiDlg*)(pApp->m_pMainWnd);
...
void ChuobiApp::SendUIMessage(int message, string data)
{
...
int ChuobiApp::SendPostThread(DWORD msgtype)
{
	ChuobiDlg *pDlg = (ChuobiDlg*)(((ChuobiApp*)this)->m_pMainWnd) ;
	if (pDlg == NULL)
	{
		return 0 ;
	}
...
void ChuobiApp::GetMainDlgStruct()
{
...
void ChuobiApp::InsertAppTransaction(string txdetail)
{
...
void  ChuobiApp::ParseUIConfigFile(const string& strExeDir){
	string configpath = "";
	configpath += strprintf("%s",strExeDir);
	configpath+= strprintf("\\%s","honghuoclient.conf");
