	...
	if( NULL != GetSafeHwnd() ) {
		...
		*/
		...
		if ( NULL != pst ) {
			CRect rect ;
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL ,(rc.Width()/100)*8 ,(rc.Height()/100)*5+4  , (rc.Width()/100)*40, /*(rc.Height()/100)*6+2*/rect.Height() ,SWP_SHOWWINDOW ); 
		...
		if ( NULL != pst ) {
			CRect rect ;
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL ,curwith,(rc.Height()/100)*5+8  , rect.Width(), rect.Height(),SWP_SHOWWINDOW ); 
			curwith+=rect.Width();
			curwith += 5;
		}
		pst = GetDlgItem( IDC_COMBO_APPID ) ;
...
BOOL CIpo::Create(CWnd* pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID)
{
	...
	if ( bRes ) {
		...
		m_rBtnSelect.SetBitmaps( IDB_BITMAP_BUTTON , RGB(255, 255, 0) , IDB_BITMAP_BUTTON , RGB(255, 255, 255) );
		m_rBtnSelect.SetAlign(CButtonST::ST_ALIGN_OVERLAP);
		m_rBtnSelect.SetWindowText("账号查询") ;
		m_rBtnSelect.SetFontEx(20 , _T("微软雅黑"));
		...
		m_rBtnDraw.SetBitmaps( IDB_BITMAP_BUTTON , RGB(255, 255, 0) , IDB_BITMAP_BUTTON , RGB(255, 255, 255) );
		m_rBtnDraw.SetAlign(CButtonST::ST_ALIGN_OVERLAP);
		m_rBtnDraw.SetWindowText("提现") ;
		m_rBtnDraw.SetFontEx(20 , _T("微软雅黑"));
