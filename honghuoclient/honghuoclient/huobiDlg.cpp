...
ChuobiDlg::ChuobiDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(ChuobiDlg::IDD, pParent)
{
...
void ChuobiDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(ChuobiDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_CREATE()
	ON_BN_CLICKED(IDC_BUTTON_MAIN, &ChuobiDlg::OnBnClickedButtonMain)
	ON_BN_CLICKED(IDC_BUTTON_SEND, &ChuobiDlg::OnBnClickedButtonSend)
	ON_BN_CLICKED(IDC_BUTTON_RECE, &ChuobiDlg::OnBnClickedButtonRece)
	ON_BN_CLICKED(IDC_BUTTON_DEAL, &ChuobiDlg::OnBnClickedButtonDeal)
	ON_BN_CLICKED(IDC_BUTTON_APP, &ChuobiDlg::OnBnClickedButtonIpo)
	ON_BN_CLICKED(IDC_BUTTON_SET, &ChuobiDlg::OnBnClickedButtonSet)
	ON_BN_CLICKED(IDC_BUTTON_CLOSE, &ChuobiDlg::OnBnClickedButtonClose)
	ON_BN_CLICKED(IDC_BUTTON_MIN, &ChuobiDlg::OnBnClickedButtonMin)
...
BOOL ChuobiDlg::OnInitDialog()
{
	...
	//}
	if ( ((ChuobiApp*)AfxGetApp())->pSplashThread != NULL)
	{ 
		((ChuobiApp*)AfxGetApp())->pSplashThread->PostThreadMessage(WM_QUIT,NULL,NULL);
	}

	return TRUE;  // 除非将焦点设置到控件，否则返回 TRUE
}

void ChuobiDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR ChuobiDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

bool ChuobiDlg::GetFileName(CString &fileName,CString strexe )
{
...
void ChuobiDlg::ShowDialog(UINT dlgid) 
{
	CRect rc , rt;
	GetClientRect(&rc);
...
int ChuobiDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
{ 
	...
	strcpy_s(nid.szTip,"honghuo"); //信息提示条 
	Shell_NotifyIcon(NIM_ADD,&nid); //在托盘区添加图标 
	ShowWindow(SW_HIDE); //隐藏主窗口 
} 

void ChuobiDlg::OnBnClickedButtonMin() 
{
	ToTray();
}

void ChuobiDlg::BlockDataProc(char *pRecBuffer, int nRecLen ,WPARAM wParam)
{
...
void ChuobiDlg::OnCloseWriteAppFee()
{/*
...
LRESULT ChuobiDlg::OnPopupBar(WPARAM wParam,LPARAM lParam) 
{
	///设置了不提示信息
	if (!theApp.m_poptips)
	{
...
LRESULT ChuobiDlg::OnShowTask(WPARAM wParam,LPARAM lParam) 
{ 
...
void ChuobiDlg::OnTimer(UINT_PTR nIDEvent)
{
...
void  ChuobiDlg::StopSever()
{
	string strCommand;
	strCommand = strprintf("%s",_T("stop"));
	string strSendData;
	string strret = _T("Honghuo server stopping");
...
{ 
	...
	nid.uCallbackMessage=WM_SHOWTASK; //自定义的消息名称 
	nid.hIcon=LoadIcon(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDR_MAINFRAME)); 
	strcpy_s(nid.szTip,"honghuo"); //信息提示条为“计划任务提醒” 
	Shell_NotifyIcon(NIM_DELETE,&nid); //在托盘区删除图标 
} 

void ChuobiDlg::ShowStateTip(UINT nButtonID)
{
