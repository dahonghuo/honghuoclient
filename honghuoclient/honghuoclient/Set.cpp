...
void CSet::OnSize(UINT nType, int cx, int cy)
{
	...
	if( NULL != GetSafeHwnd() ) {
		...
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30 , 30 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_QBJM);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30+168*1, 30 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_XGMM);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30+168*2, 30 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_QBSD);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30+168*3, 30 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_QBBF);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30+168*4, 30 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		}

		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_DCSY);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30 , 30+168*1 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_DRSY);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30+168*1, 30+168*1 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_YYID);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30+168*2, 30+168*1 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		...
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30+168*3, 30+168*1 , 168, 168  ,SWP_SHOWWINDOW ) ; 
		}
	}
}
...
BOOL CSet::Create(CWnd* pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID)
{
	...
	if ( bRes ) {
		...
		UpdateData(FALSE);
		m_rBtnRpc.SetBitmaps( IDB_BITMAP_SETRPC , RGB(255, 255, 0) , IDB_BITMAP_SETRPC , RGB(255, 255, 255) );
		...
		m_rBtnQbjm.SetBitmaps( IDB_BITMAP_SETQB , RGB(255, 255, 0) , IDB_BITMAP_SETQB , RGB(255, 255, 255) );
		...
		m_rBtnXgmm.SetBitmaps( IDB_BITMAP_SETPASS , RGB(255, 255, 0) , IDB_BITMAP_SETPASS , RGB(255, 255, 255) );
		...
		m_rBtnQbsd.SetBitmaps( IDB_BITMAP_SETQBSD , RGB(255, 255, 0) , IDB_BITMAP_SETQBSD , RGB(255, 255, 255) );
		...
		m_rBtnQbbf.SetBitmaps( IDB_BITMAP_SETQBBF , RGB(255, 255, 0) , IDB_BITMAP_SETQBBF , RGB(255, 255, 255) );
		...
		m_rBtnDcsy.SetBitmaps( IDB_BITMAP_SETDCSY , RGB(255, 255, 0) , IDB_BITMAP_SETDCSY , RGB(255, 255, 255) );
		...
		m_rBtnDrsy.SetBitmaps( IDB_BITMAP_SETDRSY , RGB(255, 255, 0) , IDB_BITMAP_SETDRSY , RGB(255, 255, 255) );
		...
		m_rBtnSzyyid.SetBitmaps( IDB_BITMAP_SETAPPID , RGB(255, 255, 0) , IDB_BITMAP_SETAPPID , RGB(255, 255, 255) );
		...
		m_rBtnFfmrsz.SetBitmaps( IDB_BITMAP_SETMRSZ , RGB(255, 255, 0) , IDB_BITMAP_SETMRSZ , RGB(255, 255, 255) );
		...
		m_rBtnGbts.SetBitmaps( IDB_BITMAP_SETGBTS , RGB(255, 255, 0) , IDB_BITMAP_SETGBTS , RGB(255, 255, 255) );
...
{
	// TODO: 在此添加控件通知处理程序代码
	ChuobiDlg *pDlg = (ChuobiDlg*)AfxGetMainWnd();
	if(!pDlg) {
		return;
	}
...
void CSet::OnBnClickedButtonDcsy()
{
	...
	// TODO: 在此添加控件通知处理程序代码
	CFileDialog dlg(FALSE,NULL,NULL,OFN_HIDEREADONLY|OFN_FILEMUSTEXIST ,_T("*.hhc||"));
	if (IDOK == dlg.DoModal())
	{
		CString strPath = dlg.GetPathName();
		if (!GetFileName(strPath,_T(".hhc")))
		{
			return;
		}
...
void CSet::OnBnClickedButtonDrsy()
{
	...
	ofn.lpstrFilter =  "All Files(*.hhc)\0*.hhc\0All Files(*.*)\0*.*\0\0"; 
	ofn.nFilterIndex = 1; 
	...
	if (::GetOpenFileName(&ofn))
	{
		...
		if (size > 0)
		{
			...
			((ChuobiDlg*)(theApp.m_pMainWnd))->ClosWalletWind();
		}else{
