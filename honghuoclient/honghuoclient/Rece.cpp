...
BOOL CRece::Create(CWnd* pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID)
{
	...
	if ( bRes ) {
		...
		m_rBtnExcel.SetBitmaps( IDB_BITMAP_BUTTON , RGB(255, 255, 0) , IDB_BITMAP_BUTTON , RGB(255, 255, 255) );
		m_rBtnExcel.SetAlign(CButtonST::ST_ALIGN_OVERLAP);
		m_rBtnExcel.SetWindowText("导出Excel") ;
		m_rBtnExcel.SetFontEx(20 , _T("微软雅黑"));
		m_rBtnExcel.SetColor(CButtonST::BTNST_COLOR_FG_OUT , RGB(255,255,255));
		m_rBtnExcel.SetColor(CButtonST::BTNST_COLOR_FG_IN , RGB(200, 75, 60));
...
void CRece::OnSize(UINT nType, int cx, int cy)
{
	...
	if( NULL != GetSafeHwnd() ) {
		CRect rc  ;
		GetClientRect( rc ) ;
		CWnd *pWnd = (CWnd*)GetDlgItem(IDC_LIST);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,20 , 20 , 860, 416  ,SWP_SHOWWINDOW ) ; 
		}

		pWnd = (CWnd*)GetDlgItem(IDC_EXPORT_EXEL2);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,880-81-81-81 - 81 - 10-10 - 10 , 416+20+10 , 81, 31  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_JHDZ);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,880-81-81-81-10-10 , 416+20+10 , 81, 31  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_NEWADDRESS);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,880-81-81-10 , 416+20+10 , 81, 31  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_COPYADDRESS);
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,880-81 , 416+20+10 , 81, 31  ,SWP_SHOWWINDOW ) ; 
		}
